...
struct MySolver : public Context {
    ...
    int max_current_order = 100;
    double warehouse_coeff = 5.0;
    int update_pool_every = 10;
    double best_deliver_dist_coeff = 0.7;
    ...
    vector<int> rank_orders() {
        ...
        const double WAREHOUSE_COEF = warehouse_coeff;
    ...
    void update_tasks_pool() {
        ...
        const int MAX_CURRENT_ORDERS = max_current_order; // heuristic
    ...
    bool make_drone_busy(int drone_id) {
        ...
        } else if (best_deliver_dist * best_deliver_dist_coeff < best_load_dist) {
            do_deliver(best_deliver_task_it, drone_id);
    ...
    void Solve() {
        ...
        const int UPDATE_POOL_EVERY = update_pool_every;  // heuristic
        update_tasks_pool();
    ...
    void ReInit() {
        ...
        current_warehouses.clear();
        drones.clear();
        potential_load_tasks_pool.clear();
        Solution.clear();
    }

    void RandomSearch(int num_iter) {
        mt19937 gen(42);

        cerr << "Global optimizations" << endl;

        uniform_int_distribution<int> max_current_order_dist(1, 1000);
        uniform_real_distribution<double> warehouse_coeff_dist(0, 10.0);
        uniform_real_distribution<double> best_deliver_dist_coeff_dist(0.1, 0.9);

        TSolution best_solution;

        uint64_t max_score = 0;
        int best_current_order = 0;
        double best_coeff = 0;
        int update_pool_best = 0;
        double best_best_deliver_coeff = 0;

        for (int i = 0; i < num_iter; ++i) {
            max_current_order = max_current_order_dist(gen);
            warehouse_coeff = warehouse_coeff_dist(gen);
            best_deliver_dist_coeff = best_deliver_dist_coeff_dist(gen);
            ReInit();
            Solve();
            ...
            if (score > max_score) {
                ...
                max_score = score;
                best_current_order = max_current_order;
                best_coeff = warehouse_coeff;
                best_best_deliver_coeff = best_deliver_dist_coeff;
                cerr << "Iteration " << i << " score " << max_score << endl;
            }
        }

        cerr << "Best values: " << best_current_order << " " << best_coeff << " " << best_best_deliver_coeff << endl;
        Solution = best_solution;

        cerr << "Local fluctuations" << endl;
        max_current_order = best_current_order;
        warehouse_coeff = best_coeff;
        best_deliver_dist_coeff = best_best_deliver_coeff;

        int best_diff = 0;
        for (int i = -10; i < 10; ++i) {
            max_current_order = max(0, best_current_order + i);
            ReInit();
            Solve();
            uint64_t score = GetScore();
            if (score > max_score) {
        ...
        max_current_order = max(0, best_current_order + best_diff);

        for (int i = -10; i < 10; ++i) {
            warehouse_coeff = max(0., best_coeff + double(i) / 10);
            ReInit();
            Solve();
            uint64_t score = GetScore();
            if (score > max_score) {
        ...
        warehouse_coeff = max(0., best_coeff + double(best_diff) / 10);

        for (int i = -10; i < 10; ++i) {
            best_deliver_dist_coeff = max(0., best_best_deliver_coeff + double(i) / 10);
            ReInit();
            Solve();
            uint64_t score = GetScore();
            if (score > max_score) {
        ...
        best_deliver_dist_coeff = max(0., best_best_deliver_coeff + double(best_diff) / 10);
        cerr << "Best values: " << max_current_order << " " << warehouse_coeff << " " << best_deliver_dist_coeff << endl;
...
int main(int argc, char** argv) {
    ...
    solver.RandomSearch(atoi(argv[1]));
    cerr << "Done!" << endl;
